...
def arg_subtype_med(input1,input2):
    ...
    for mat, subtype in zip(input1.values.tolist(),arg_subtype):   
        ...
        # print("test9",line)
        # ['4902', 'Alcohol', '117', '135', 'StatusTime', '108', '116', 'Previous']
        temp = line[0]+' '+line[2]+' '+line[3]+' '+line[5]+' '+ line[6]+' '+subtype
...
def main():
    ...
    match_med = pd.read_csv('./experiments/system5/piece_subtype/subtype_temp_med.csv',header = None)
    match_emp = pd.read_csv('./experiments/system5/piece_subtype/subtype_temp_emp.csv',header = None)
    match_liv_status = pd.read_csv('./experiments/system5/piece_subtype/subtype_temp_liv_status.csv',header = None)
    match_liv_type = pd.read_csv('./experiments/system5/piece_subtype/subtype_temp_liv_type.csv',header = None)
    
    with open("./relation_pred/temp-base-pred-subtype-med-123.csv", 'r') as iFile_pred1:
        med_pred = iFile_pred1.read()   
    med_pred_ = med_pred.splitlines()
    
    with open("./relation_pred/temp-base-pred-subtype-emp-123.csv", 'r') as iFile_pred2:
        emp_pred = iFile_pred2.read()   
    emp_pred_ = emp_pred.splitlines()
    
    with open("./relation_pred/temp-base-pred-subtype-liv-status-123.csv", 'r') as iFile_pred3:
        liv_status_pred = iFile_pred3.read()   
    liv_status_pred_ = liv_status_pred.splitlines()
    
    with open("./relation_pred/temp-base-pred-subtype-liv-type-123.csv", 'r') as iFile_pred4:
        liv_type_pred = iFile_pred4.read()   
    liv_type_pred_ = liv_type_pred.splitlines()
    
    with open(input_relation,'r') as iFile_rel: # all-poss-relation.txt"
        eve_arg_relation = iFile_rel.read()
        
    all_relation = eve_arg_relation.splitlines() #'0477 Employment 34 44 StatusEmploy 25 31\tworked'
    
    subtype_livs_list = ['current','future','none','past']
    subtype_livt_list = ['alone','homeless','with_family','with_others']
    
    #print('test5',match_med[4], med_pred_)
    ...
    print("test3", all_subtype)
    #print("all_relation",all_relation)


    for filename in glob.glob('./experiments/'+ event_table_dir +'/ann/*.ann'):
       ...
       # print(ann_trigger)
        ...
        for sb in all_subtype:
            ...
            #sbb ['2633', '392', '396', '384', '391', 'past']
            #print("sbb", sbb)

            if sbb[0] == idx[1]:
                print('test2, file name',sbb, sbb[0], idx[1])
                match_subtypes.append(sbb[0]+' '+sbb[1]+' '+sbb[2]+' '+sbb[3]+' '+sbb[4]+' '+sbb[5])           
        ...
        for a in ann_all: # trigger ann           
            ...
            for argument_line in all_relation: # 0446 Drug 70 83 Type 64 77    Crack Cocaine
                ...
                if argument_line.strip().split()[0] == idx[1]:
                    ...
                    event_start = int(argument_line.strip().split()[2]) # trigger start 
                    event_end = int(argument_line.strip().split()[3]) # trigger end
                    
                    argument_type = argument_line.strip().split()[4] # argument
                    argument_start = int(argument_line.strip().split()[5]) # argument                     
                    argument_end = int(argument_line.strip().split()[6]) # argument 
                    argument_content = argument_line.strip().split()[4:]
                    
                    #print("test8", argument_content)
                    ...
                    if event_type == EVNN[0] and event_start == int(EVNN[1]) and event_end == int(EVNN[2]):
                        ...
                        if "Status" in argument_type:
                            print("test3", argument_type)
                            # argument_type = "Status"
                            
                            for match_subtype in match_subtypes:
                                print('test5', match_subtype.strip().split()[5])
                                match_arg_start = int(match_subtype.strip().split()[3])
                                match_arg_end = int(match_subtype.strip().split()[4])
                                match_trg_start = int(match_subtype.strip().split()[1])
                                match_trg_end = int(match_subtype.strip().split()[2])

                                if match_trg_start == event_start and match_trg_end == event_end and match_arg_start == argument_start and match_arg_end == argument_end and match_subtype.strip().split()[5] not in subtype_livt_list:    
                                    
                                    print("test1",match_subtype.strip().split(),argument_content[1],match_subtype.strip().split()[2],argument_content[2],match_subtype.strip().split()[3])
                                    ...
                                    if not ''.join('TypeLivingVal'+' ' +'T'+ str(argument_num)) in argument_subT:
                                        ...
                                        if argument_content[0] == 'StatusEmploy':
                                            list_subtype = 'A'+ str(sub_num) +'\t'+ 'StatusEmployVal'+' ' +'T'+ str(argument_num) +' '+match_subtype.strip().split()[5]
                                    
                                        else:                     
                                            list_subtype = 'A'+ str(sub_num) +'\t'+ 'StatusTimeVal'+' ' +'T'+ str(argument_num) +' '+match_subtype.strip().split()[5]                                       
                                            
                                        list_subtypes.append(list_subtype)
                         
                        print("test",list_subtypes)  
                            
                        # A2    StatusTimeVal T3 current
                        # StatusEmployVal, StatusTimeVal,TypeLivingVal 
                        if "TypeLiving" in argument_type:
                            # argument_type = "Type"
                            for match_subtype2 in match_subtypes:
                                
                                match_arg_start = int(match_subtype2.strip().split()[3])
                                match_arg_end = int(match_subtype2.strip().split()[4])
                                match_trg_start = int(match_subtype2.strip().split()[1])
                                match_trg_end = int(match_subtype2.strip().split()[2])

                                #if match_subtype.strip().split()[1] == argument_content[1] and match_subtype.strip().split()[2] == argument_content[2]:  
                                if match_trg_start == event_start and match_trg_end == event_end and match_arg_start == argument_start and match_arg_end == argument_end and match_subtype2.strip().split()[5] in subtype_livt_list:
                                    if not ''.join('TypeLivingVal'+' ' +'T'+ str(argument_num)) in argument_subT:
                                        sub_num += 1
                                        argument_subT.add(''.join('TypeLivingVal'+' ' +'T'+ str(argument_num)))
                                        
                                        list_subtype = 'A'+ str(sub_num) +'\t'+ 'TypeLivingVal'+' ' +'T'+ str(argument_num) +' '+match_subtype2.strip().split()[5]
                                        list_subtypes.append(list_subtype)
