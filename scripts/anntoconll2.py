...
def text_to_conll(f):
    ...
    if options.nosplit:
        ...
        for l in f:
            ...
            # print('test3.3', l,sentences)
        
        l = sentencebreaks_to_newlines(' '.join(add_sent).replace("\n", ""))
        sentences.extend([s for s in NEWLINE_TERM_REGEX.split(l) if s])
        # m = ' '.join(add_sent)
        # m = txt.replace("\n", "")
        # print("test3.1", ' '.join(add_sent).replace("\n", ""))
        
        # m = sentencebreaks_to_newlines(' '.join(add_sent).replace("\n", ""))
        #print("test3.4",l,sentences)
    ...
    for s in sentences:
        ...
        #print("test1", TOKENIZATION_REGEX.split(s))
