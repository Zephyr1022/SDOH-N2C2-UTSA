...
def main():
    ...
    match_med = pd.read_csv('./template_rl/subtype_test12_med.csv',header = None)
    match_emp = pd.read_csv('./template_rl/subtype_test12_emp.csv',header = None)
    match_liv_status = pd.read_csv('./template_rl/subtype_test12_liv_status.csv',header = None)
    match_liv_type = pd.read_csv('./template_rl/subtype_test12_liv_type.csv',header = None)
    
    with open("relation_pred/test12-base-pred-subtype-med-123.csv", 'r') as iFile_pred1:
        med_pred = iFile_pred1.read()   
    med_pred_ = med_pred.splitlines()
    
    with open("relation_pred/test12-base-pred-subtype-emp-123.csv", 'r') as iFile_pred2:
        emp_pred = iFile_pred2.read()   
    emp_pred_ = emp_pred.splitlines()
    
    with open("relation_pred/test12-base-pred-subtype-liv-status-123.csv", 'r') as iFile_pred3:
        liv_status_pred = iFile_pred3.read()   
    liv_status_pred_ = liv_status_pred.splitlines()
    
    with open("relation_pred/test12-base-pred-subtype-liv-type-123.csv", 'r') as iFile_pred4:
    ...
    for filename in glob.glob(event_table_dir):
        ...
        filename2 = filename.replace("test", "test_table") # copy new
        #print(filename2)
        ...
        for a in ann_all: # trigger ann           
            ...
            for argument_line in all_relation: # 0446 Drug 70 83 Type 64 77    Crack Cocaine
                ...
                if argument_line.strip().split()[0] == idx[0]:
