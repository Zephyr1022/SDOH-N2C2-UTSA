...
def main():
    ...
    match_med = pd.read_csv('./template_rl/subtype_test11_med.csv',header = None)
    match_emp = pd.read_csv('./template_rl/subtype_test11_emp.csv',header = None)
    match_liv_status = pd.read_csv('./template_rl/subtype_test11_liv_status.csv',header = None)
    match_liv_type = pd.read_csv('./template_rl/subtype_test11_liv_type.csv',header = None)
    
    with open("relation_pred/test11-base-pred-subtype-med-123.csv", 'r') as iFile_pred1:
        med_pred = iFile_pred1.read()   
    med_pred_ = med_pred.splitlines()
    
    with open("relation_pred/test11-base-pred-subtype-emp-123.csv", 'r') as iFile_pred2:
        emp_pred = iFile_pred2.read()   
    emp_pred_ = emp_pred.splitlines()
    
    with open("relation_pred/test11-base-pred-subtype-liv-status-123.csv", 'r') as iFile_pred3:
        liv_status_pred = iFile_pred3.read()   
    liv_status_pred_ = liv_status_pred.splitlines()
    
    with open("relation_pred/test11-base-pred-subtype-liv-type-123.csv", 'r') as iFile_pred4:
