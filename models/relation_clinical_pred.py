...
device_cuda = sys.argv[3]
test_pred_loc = sys.argv[4]
test_prob_loc = sys.argv[5]
...
def validating(model, dataloader, device, epoch, epochs):
    ...
    for data in dataloader:
        ...
        for a,b in zip(y_true, results):
            ...
            #logits = logits.detach().cpu()
        for a in torch.sigmoid(logits).float().to('cpu'):
            all_probs.append(a)
    ...
    return all_preds, avg_val_f1, all_probs
...
def main():
    ...
    X_train_txt = []
    y_train = []
    with open('./template_rl/relation_train.csv') as iFile:
    ...
    X_val_txt = []
    y_val = []
    with open('./template_rl/relation_dev.csv') as iFile:
        iCSV = csv.reader(iFile, delimiter=',')
        #header = next(iCSV)
        #print(header)
        for row in iCSV:
    ...
    model_Base.load_state_dict(torch.load(MODEL_PATH)) # model load
    test_preds, test_f1, test_probs = validating(model_Base, data_loader_test, device, epoch, epochs)
    
    print("test_f1:", test_f1)
    
    with open(test_pred_loc, 'w') as out_file: #'relation_pred/test-base-pred-V1.csv'
        for row in test_preds:
            out_file.write(f"{row}\n")
            
    with open(test_prob_loc, 'w') as out_file: #'relation_pred/test-base-prob-V1.csv'
