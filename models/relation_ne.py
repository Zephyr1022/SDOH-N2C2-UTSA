...
'''
The outdoors <Drug> <type> Retired <\type> assistant to nutrition< <\Drug>.
...
def event_argument(input_file): 
    ...
    for evtp in event_types:# put same event togethers but only generate one data
        relations = []
        argument_out = []
        event_out  = []

        for id_event in sent_list: # outside loop E3, E4, E5, E6 (Drug)
            if "E" in id_event: # Loop Events(E1,E2,...): E1 Drug
                ...
                if dataset[id_event].split()[1].split(":")[0] == evtp:
                    #print(dataset[id_event].split()[0:])
...
def main():
    ...
    regex = re.compile(r'\d+')
        ...
        #trigger_filename = filename.replace('./Annotations/'+para_data['argument_train_dev']+'/mimic', "./Annotations/train/test") 
        # input -> output
        # print(txt_filename)
    
    for filename in glob.glob('./Annotations/'+ train_dev_test_dir +'/temp/*.ann'): #./Annotations/train/mimic/*.ann'

        txt_filename = filename.replace(".ann", ".txt")
        idx = regex.findall(filename)
        events,arguments = event_argument(filename) # all relations b/t drug and arguments
        #print(events[5],arguments[5])  # extract the corresponding event trigger and arguments
        ...
        with open(txt_filename, 'r') as iFile:
            data = iFile.read()
            ...
            for rl in relations:
                ...
                examples.append(example[0]) # list -> string
    ...
    #print(len(examples))
    #print(len(examples))
    list_match = ['not match' for i in range(len(examples))]
    ...
    df.to_csv('relation_'+ train_dev_test_dir +'_notmatch.csv', header = False) # relative position
    print(len(examples))
